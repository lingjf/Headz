#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdarg.h>
#include <assert.h>

#include "bitopz.h"

#ifdef __GNUC__

#else

int __ffs(int x)
{
   int i;
   assert(&x);

   for (i=0; i<BITS_PER_LONG; i++) {
      if (x & (1UL<<i))
      return i+1;
   }
   return 0;
}

int __fls(int x)
{
   int i;
   assert(&x);

   for (i=BITS_PER_LONG-1; i>=0; i--) {
      if (x & (1UL<<i))
      return i+1;
   }
   return 0;
}

#endif

unsigned long __bit_weight(unsigned long x)
{
   unsigned long n = 0;
   assert(&x);
   while (x) {
      x = x & (x - 1);
      n++;
   }
   return n;
}

unsigned long bitweight32(unsigned long w)
{
   register unsigned long res;
   assert(&w);
   res = w - ((w >> 1) & 0x55555555);
   res = (res & 0x33333333) + ((res >> 2) & 0x33333333);
   res = (res + (res >> 4)) & 0x0F0F0F0F;
   res = res + (res >> 8);
   return (res + (res >> 16)) & 0x000000FF;
}

unsigned long bitweight16(unsigned long w)
{
   register unsigned long res;
   assert(&w);
   res = w - ((w >> 1) & 0x5555);
   res = (res & 0x3333) + ((res >> 2) & 0x3333);
   res = (res + (res >> 4)) & 0x0F0F;
   return (res + (res >> 8)) & 0x00FF;
}

unsigned long bitweight8(unsigned long w)
{
   register unsigned long res;
   assert(&w);
   res = w - ((w >> 1) & 0x55);
   res = (res & 0x33) + ((res >> 2) & 0x33);
   return (res + (res >> 4)) & 0x0F;
}


static const unsigned char byte_rev_table[256] = {
	,
};

unsigned char bitrev8(unsigned char x)
{
   assert(&x);
   return byte_rev_table[x];
}

unsigned short bitrev16(unsigned short x)
{
   assert(&x);
   return (byte_rev_table[x & 0xff] << 8) | byte_rev_table[x >> 8];
}

unsigned long bitrev32(unsigned long x)
{
   assert(&x);
   return (byte_rev_table[x & 0xff] << 24) | (byte_rev_table[(x >> 8) & 0xff]) << 16
               | (byte_rev_table[(x >> 16) & 0xff]) << 8 | (byte_rev_table[(x >> 24) & 0xff]);
}

void bitrev(unsigned char *buf, int size)
{
   unsigned char *i = buf, *j = buf + size - 1;
   assert(buf);
   assert(size >= 0);

   while (i < j) {
      unsigned char t = *i;
      *i = byte_rev_table[*j];
      *j = byte_rev_table[t];
      i++;
      j--;
   }
   if (i == j)
      *i = byte_rev_table[*j];
}

